...
const GraphModal: React.FC<GraphModalProps> = ({ 
    ...
    const handleNodeClick = useCallback((node: NodeSingular) => {
        ...
        // Only handle document and image clicks, not tag clicks
        if ((nodeData.type === 'document' || nodeData.type === 'image') && nodeData.filePath) {
            if (onNavigateToFile) {
                ...
                style: [
                    {
                        ...
                        style: {
                            'background-color': (ele: cytoscape.NodeSingular) => {
                                ...
                                if (data.type === 'tag') return colors.tag;
                                if (data.type === 'image') return getComputedStyle(document.documentElement).getPropertyValue('--graph-color-image') || '#4bb3fd';
                            ...
                            'width': (ele: cytoscape.NodeSingular) => {
                                const data = ele.data();
                                if (data.type === 'tag') return '30px';
                                if (data.type === 'image') return '28px';
                                return data.isCurrent ? '35px' : '30px';
                            },
